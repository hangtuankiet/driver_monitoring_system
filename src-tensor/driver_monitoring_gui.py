...
class DriverMonitoringGUI:
    ...
    def setup_gui(self):
        self.root.title("🚗 Smart Driver Monitoring System")  # Changed to English
    ...
    def create_status_indicators(self):
        ...
        self.lbl_status = Label(status_frame, text="🚗 Status: Idle", font=("Arial", 14, "bold"), fg="#263238",
                                bg="#ECEFF1")  # Changed "Trạng thái: Chờ" to "Status: Idle"
        self.lbl_status.pack(pady=10)
        self.lbl_eye = Label(status_frame, text="👀 Eyes: --", font=("Arial", 14), fg="#263238", bg="#ECEFF1")  # Changed "Mắt" to "Eyes"
        self.lbl_eye.pack()
        self.lbl_yawn = Label(status_frame, text="🗣️ Yawn: --", font=("Arial", 14), fg="#263238", bg="#ECEFF1")  # Changed "Ngáp" to "Yawn"
        self.lbl_yawn.pack()
    ...
    def create_control_buttons(self):
        ...
        self.btn_monitor = ttk.Button(btn_frame, text="▶ Start", command=self.toggle_monitoring,
                                      style="Green.TButton")  # Changed "Bắt đầu" to "Start"
        ...
        self.btn_evaluate = ttk.Button(btn_frame, text="📊 Evaluate Performance", command=self.toggle_evaluation,
                                       style="Purple.TButton")  # Changed "Đánh giá hiệu năng" to "Evaluate Performance"
    ...
    def toggle_monitoring(self):
        if not self.is_monitoring:
            ...
            self.is_paused = False
            self.btn_monitor.config(text="⏸ Pause")  # Changed "Tạm dừng" to "Pause"
            self.lbl_status.config(text=f"🚗 Status: Monitoring ({self.logic.get_source_type()})")  # Changed "Trạng thái: Đang giám sát" to "Status: Monitoring"
            self.update_video_thread()
        else:
            self.is_paused = True
            self.btn_monitor.config(text="▶ Resume")  # Changed "Tiếp tục" to "Resume"
            self.lbl_status.config(text="🚗 Status: Paused")  # Changed "Trạng thái: Tạm dừng" to "Status: Paused"

    def show_start_options(self):
        top = tk.Toplevel(self.root)
        top.title("Select Input Source")  # Changed "Chọn nguồn đầu vào" to "Select Input Source"
    ...
    def start_video_selection(self, top=None):
        video_path = filedialog.askopenfilename(
            title="Select Video",  # Changed "Chọn video" to "Select Video"
    ...
    def toggle_evaluation(self):
        if not self.logic.is_evaluating:
            video_path = filedialog.askopenfilename(
                title="Select Video for Evaluation",  # Changed "Chọn video để đánh giá" to "Select Video for Evaluation"
                initialdir=self.logic.config['video_path'],
                filetypes=[("Video files", "*.mp4 *.avi *.mov")]
            )
            if not video_path:
            ...
            ground_truth_input = tk.simpledialog.askstring("Enter Ground Truth",  # Changed "Nhập Ground Truth" to "Enter Ground Truth"
            ...
            if ground_truth_input:
                try:
                    ...
                    messagebox.showerror("Error", "Invalid ground truth format!")  # Changed "Lỗi" and message to English
                    return
            else:
                ground_truth = []
            ...
            if success:
                self.is_monitoring = True
                self.btn_evaluate.config(text="⏹ Stop Evaluation")  # Changed "Dừng đánh giá" to "Stop Evaluation"
                self.btn_monitor.config(state="disabled")
                self.lbl_status.config(text="🚗 Status: Evaluating (Video)")  # Changed "Trạng thái: Đang đánh giá" to "Status: Evaluating"
    ...
    def stop_evaluation(self):
        ...
        self.lbl_status.config(text="🚗 Status: Idle")  # Changed "Trạng thái: Dừng" to "Status: Idle"
        if hasattr(self, 'eval_info_label') and self.eval_info_label:
            self.eval_info_label.place_forget()
        if final_stats is not None:
            messagebox.showinfo("Success", "Performance evaluation completed. Results saved!")  # Changed "Hoàn tất" and message to English
        ...
        def update():
            while self.logic.is_monitoring and not self.is_paused:
                ...
                if self.logic.is_evaluating and hasattr(self, 'eval_info_label') and self.eval_info_label:
                    ...
                    self.root.after(0, lambda: self.eval_info_label.config(text=f"⏱ Evaluation: {int(remaining)}s | FPS: {self.logic.get_fps()}"))  # Changed "Đánh giá" to "Evaluation"
    ...
    def show_settings(self):
        ...
        tk.Label(settings_window, text="Camera/Video Settings", font=("Arial", 12, "bold"), fg="#263238", bg="#ECEFF1").pack(pady=10)  # Changed "Cài đặt Camera/Video"
        ...
        tk.Label(settings_window, text="Default Video Path:", fg="#263238", bg="#ECEFF1").pack()  # Changed "Đường dẫn video mặc định"
        video_path = ttk.Entry(settings_window)
        video_path.insert(0, self.logic.config['video_path'])
        video_path.pack()

        tk.Label(settings_window, text="Alert Settings", font=("Arial", 12, "bold"), fg="#263238", bg="#ECEFF1").pack(pady=10)  # Changed "Cài đặt Cảnh báo"
        tk.Label(settings_window, text="Eye Closure Threshold (seconds):", fg="#263238", bg="#ECEFF1").pack()  # Changed "Ngưỡng thời gian nhắm mắt (giây)"
        eye_threshold = ttk.Entry(settings_window)
        eye_threshold.insert(0, str(self.logic.config['eye_closure_threshold']))
        eye_threshold.pack()

        tk.Label(settings_window, text="Audio Settings", font=("Arial", 12, "bold"), fg="#263238", bg="#ECEFF1").pack(pady=10)  # Changed "Cài đặt Âm thanh"
        sound_enabled_var = tk.BooleanVar(value=self.logic.config['sound_enabled'])
        tk.Checkbutton(settings_window, text="Enable Alert Sound", variable=sound_enabled_var, fg="#263238", bg="#ECEFF1").pack(pady=5)  # Changed "Bật âm thanh cảnh báo"
        tk.Label(settings_window, text="Volume:", fg="#263238", bg="#ECEFF1").pack()  # Changed "Âm lượng"
        ...
        tk.Checkbutton(settings_window, text="Save Alert History", variable=save_alerts_var, fg="#263238", bg="#ECEFF1").pack(pady=10)  # Changed "Lưu lịch sử cảnh báo"
        ...
        def save_settings():
            try:
                ...
                messagebox.showinfo("Success", "Settings saved successfully!")  # Changed "Thành công" and message
                settings_window.destroy()
            except ValueError as e:
                messagebox.showerror("Error", "Invalid value!")  # Changed "Lỗi" and message

        ttk.Button(settings_window, text="Save Settings", command=save_settings, style="Green.TButton").pack(pady=20)  # Changed "Lưu cài đặt"

    def show_alerts(self):
        alerts_window = tk.Toplevel(self.root)
        alerts_window.title("Alert History")  # Changed "Lịch sử Cảnh báo"
        alerts_window.geometry("600x400")
        alerts_window.configure(bg="#ECEFF1")

        columns = ('Time', 'Alert', 'Eye Closure Time')  # Changed "Thời gian", "Cảnh báo", "Thời gian nhắm mắt"
        ...
        try:
            ...
            tree.insert('', 'end', values=('No data', '', ''))  # Changed "Không có dữ liệu"
        ...
        def export_alerts():
            try:
                ...
                messagebox.showinfo("Success", f"Data exported to {export_file}")  # Changed "Thành công" and message
            except Exception as e:
                messagebox.showerror("Error", f"Error exporting data: {str(e)}")  # Changed "Lỗi" and message

        ttk.Button(alerts_window, text="Export to CSV", command=export_alerts, style="Blue.TButton").pack(pady=10)  # Changed "Xuất CSV"

    def on_closing(self):
        if messagebox.askokcancel("Exit", "Do you want to exit?"):  # Changed "Thoát" and message
            self.logic.stop_monitoring()
            self.root.quit()
